...
/*   Updated: 2026/09/01 15:31:07 by zkarman          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pixel_to_circle(t_bible *master, double *world, int *pt)
{
	world[0] = master->player.x + ((double)pt[0] / master->map.tile_size);
	world[1] = master->player.y + ((double)pt[1] / master->map.tile_size);
	put_pixel_to_frame(&master->img, master->mini.mid[0] + pt[0],
		master->mini.mid[1] + pt[1],
...
void	update_minimap(t_bible *master)
{
	...
	pt[1] = -master->mini.rad;
	while (pt[1] <= master->mini.rad)
	{
		pt[0] = -master->mini.rad;
		while (pt[0] <= master->mini.rad)
		{
			dist_sq = (pt[0] * pt[0]) + (pt[1] * pt[1]);
			if (dist_sq <= master->mini.rad * master->mini.rad
				&& dist_sq >= (master->mini.rad - 2) * (master->mini.rad - 2))
				put_pixel_to_frame(&master->img, master->mini.mid[0] + pt[0],
					master->mini.mid[1] + pt[1], 0x555555);
			else if (dist_sq < (master->mini.rad - 2) * (master->mini.rad - 2))
				pixel_to_circle(master, world, pt);
			pt[0]++;
		}
		pt[1]++;
	}
	draw_player_pos(master, master->mini.mid[0], master->mini.mid[1], 0xFF0000);
}
